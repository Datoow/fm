...
void ffm_fit_sgd(ffm_coef *coef, cs *A, ffm_vector *y, ffm_param *param) {
  ...
  for(int f = 0; f < n_features; f++){
      ...
      if (t > 1.0) ww[f] = 1.0;
      else if (t < -1.0) ww[f] = -1.0;
      else ww[f] = t;
      
      if (t > 0.0)       
        ffm_vector_set( coef->w, f, 1.0);     
      else    
        ffm_vector_set( coef->w, f, -1.0);    
      //printf("%f %f\n", ffm_vector_get(coef->w,f), ffm_vector_get(coef_t.w,f));
  }
  ...
  for(int j = 0; j < k; j++){
    for(int f = 0; f < n_features; f++){
        ...
        if (t > 1.0) VV[j][f] = 1.0;
        else if (t < -1.0) VV[j][f] = -1.0;
        else VV[j][f] = t;
      
        if (t > 0.0)           
            ffm_matrix_set( coef->V, j, f, 1.0);      
        else
            ffm_matrix_set( coef->V, j, f, -1.0);       
    }
  } 
  ...
  for (int i = 0; i < param->n_iter; i++) {
    ...
    for (p = Ap[sample_row]; p < Ap[sample_row + 1]; p++) {
      ...
      double tilde_w = ww[Ai[p]];
      double diff = tilde_w - step_size * (y_err * Ax[p] + theta_w * coef->lambda_w);
      if(diff > 1.0) diff = 1.0;
      if(diff < -1.0) diff = -1.0;
      ww[Ai[p]] = diff;
      if (diff > 0.0)           
            ffm_vector_set( coef->w, Ai[p], 1.0);      
      else
            ffm_vector_set( coef->w, Ai[p], -1.0);   
    }
    ...
    if (k > 0)
      ...
      for (int f = 0; f < k; f++) {
        ...
        for (p = Ap[sample_row]; p < Ap[sample_row + 1]; p++) {
          ...
          double tilde_v = VV[f][Ai[p]];
          double grad = Ax[p] * cache - (Ax[p] * Ax[p]) * v;
          double lambda_V_f = ffm_vector_get(coef->lambda_V, f);
          double diff = tilde_v - step_size * (y_err * grad + v * lambda_V_f);
          if(diff > 1.0) diff = 1.0;
          if(diff < -1.0) diff = -1.0;
          VV[f][Ai[p]] = diff;
          if (diff > 0.0)           
                ffm_matrix_set( coef->V, f, Ai[p], 1.0);      
          else
                ffm_matrix_set( coef->V, f, Ai[p], -1.0);
